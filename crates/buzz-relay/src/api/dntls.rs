        ...
        DntlsAdmission::Auto | DntlsAdmission::Approve => {
            match state
                ...
                .upsert_dntls_approved_application(
                    tenant.community(), pubkey_hex, fqdn, pubkey_hex, is_admin,
                )
                .await
...
mod tests {
    ...
    async fn dntls_listed_admin_bypasses_approval_but_other_names_do_not() {
        for mode in [DntlsAdmission::Approve, DntlsAdmission::Auto] {
            ...
            Arc::get_mut(&mut state).expect("unique state").config.dntls_admins =
                vec!["josh.dntls".to_string()];
            ...
            let response = send(
                state.clone(), &host, Method::GET, PENDING_PATH, &admin, String::new(),
            ).await;
            assert_eq!(response.status(), StatusCode::OK);
            assert_eq!(read_json(response).await["applications"], serde_json::json!([]));

            let (ok, _) =
                auth_connection(state.clone(), &host, &newcomer, Some("newcomer.dntls")).await;
            assert_eq!(ok, mode == DntlsAdmission::Auto);
            let community = state.db.lookup_community_by_host(&host).await
                .expect("lookup").expect("community");
            let members = state.db.list_relay_members(community.id).await.expect("members");
            assert_eq!(
                members.iter().find(|m| m.pubkey == admin.public_key().to_hex())
                    .expect("admin member").role,
                "admin"
            );
            let newcomer_member = members.iter()
            ...
            if mode == DntlsAdmission::Auto {
                ...
                assert_eq!(json["applications"][0]["pubkey"], newcomer.public_key().to_hex());
            }

            // Exact names only: a subname does not inherit the configured role.
            let subname = Keys::generate();
            let (ok, _) = auth_connection(
                state.clone(), &host, &subname, Some("buzz.josh.dntls"),
            ).await;
            assert_eq!(ok, mode == DntlsAdmission::Auto);
            let member = state.db.get_relay_member(
                community.id, &subname.public_key().to_hex(),
            ).await.expect("subname membership");
            assert_eq!(member.map(|m| m.role).as_deref(),
                if mode == DntlsAdmission::Auto { Some("member") } else { None });
        }
    }
    ...
    async fn dntls_listed_admin_promotes_member_and_preserves_owner() {
        for initial_role in ["member", "owner"] {
            ...
            let mut state = dntls_test_state_on(&host, DntlsAdmission::Approve, TEST_REDIS_URL)
                .await.expect("requires reachable Postgres, Redis, and relay test state");
            Arc::get_mut(&mut state).expect("unique state").config.dntls_admins =
                vec!["josh.dntls".to_string()];
            ...
            let community = state.db.lookup_community_by_host(&host).await
                .expect("lookup").expect("community");
            state.db.add_relay_member(community.id, &hex, initial_role, None)
                .await.expect("seed member");
            // Also cover a previously queued application when the allowlist changes.
            state.db.upsert_dntls_pending_application(community.id, &hex, "josh.dntls")
                .await.expect("seed pending");
            let response = send_with_dntls(
                state.clone(), &host, Method::GET, PENDING_PATH, &keys, String::new(),
                Some("josh.dntls"),
            ).await;
            assert_eq!(response.status(), StatusCode::OK, "NIP-98 admission");
            assert_eq!(read_json(response).await["applications"], serde_json::json!([]));
            let expected_role = if initial_role == "owner" { "owner" } else { "admin" };
            assert_eq!(state.db.get_relay_member(community.id, &hex).await
                .expect("lookup").expect("member").role, expected_role);

            // Removing configuration is not a revocation operation.
            Arc::get_mut(&mut state).expect("unique state").config.dntls_admins.clear();
            let (ok, messages) =
    ...
    async fn dntls_admin_rebinding_transfers_role_and_publishes_both_members() {
        for mode in [DntlsAdmission::Approve, DntlsAdmission::Auto] {
            ...
            for (keys, role) in [(&first, "member"), (&second, "admin")] {
                assert_eq!(state.db.get_relay_member(
                    community.id, &keys.public_key().to_hex(),
                ).await.expect("lookup").expect("member").role, role);
            }
            let response = send(
                state.clone(), &host, Method::GET, PENDING_PATH, &first, String::new(),
            ).await;
            assert_eq!(response.status(), StatusCode::FORBIDDEN, "old key loses admin API access");
            let response = send(
            ...
            let snapshot = events.as_array().expect("events").iter()
                .find(|event| event["kind"] == 13534).expect("membership snapshot");
            for (keys, role) in [(&first, "member"), (&second, "admin")] {
                assert!(snapshot["tags"].as_array().expect("tags").iter().any(|tag| {
                    tag[0] == "member" && tag[1] == keys.public_key().to_hex() && tag[2] == role
                }), "snapshot missing {role}: {snapshot}");
            }
            // Owner bootstrap remains authoritative even when its name is displaced.
            state.db.bootstrap_owner(community.id, &second.public_key().to_hex())
                .await.expect("bootstrap owner");
            let third = Keys::generate();
            let (ok, messages) =
                auth_connection(state.clone(), &host, &third, Some("josh.dntls")).await;
            assert!(ok, "new key AUTH: {messages:?}");
            assert_eq!(state.db.get_relay_member(
                community.id, &second.public_key().to_hex(),
            ).await.expect("lookup").expect("owner").role, "owner");
        }
    }
