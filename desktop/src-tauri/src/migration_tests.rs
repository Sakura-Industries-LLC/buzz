...
fn canonical_dev_data_dir_replaces_last_component() {
    let current =
        PathBuf::from("/Users/me/Library/Application Support/net.dntls.buzz.dev.my-branch");
    let canonical = canonical_dev_data_dir(&current).unwrap();
    assert_eq!(
        canonical,
        PathBuf::from("/Users/me/Library/Application Support/net.dntls.buzz.dev")
    );
}
...
#[test]
fn legacy_app_data_dir_ignores_dntls_identifier() {
    let current = PathBuf::from("/Users/me/Library/Application Support/net.dntls.buzz");
...
fn setup_sync_layout() -> (tempfile::TempDir, PathBuf, PathBuf) {
    ...
    let worktree = parent.path().join("net.dntls.buzz.dev.my-branch");
    let main_instance = parent.path().join("net.dntls.buzz.dev.main");
...
fn seed_up_migrates_sibling_file_to_canonical_then_symlinks() {
    ...
    let sibling = canonical.parent().unwrap().join("net.dntls.buzz.dev.main");
...
fn canonical_dev_data_dir_returns_self_for_canonical_instance() {
    ...
    // testing only.
    let current = PathBuf::from("/Users/me/Library/Application Support/net.dntls.buzz.dev");
...
fn sync_migrates_teams_from_sibling_to_canonical() {
    let (_parent, canonical, worktree) = setup_sync_layout();
    let main_instance = canonical.parent().unwrap().join("net.dntls.buzz.dev.main");
